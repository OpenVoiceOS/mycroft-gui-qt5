...
manjaro() {
    ...
    echo ""
    echo "Please Enter Authentication For Installing System Dependencies"
    sudo pacman -S git cmake extra-cmake-modules-git kio kio-extras plasma-framework qt6-websockets qt6-webview qt6-declarative qt6-multimedia qt6-quickcontrols2 qt6-webengine qt6-base
    build_gui   
}
...
function build_gui() {
    ...
    cd build-testing
    cmake .. -DBUILD_WITH_QT6=On -DQT_MAJOR_VERSION=6 -DCMAKE_INSTALL_PREFIX=/usr -DCMAKE_BUILD_TYPE=Release -DKDE_INSTALL_LIBDIR=lib -DKDE_INSTALL_USE_QT_SYS_PATHS=ON -DCMAKE_CXX_COMPILER=clazy
    make -j4
    sudo make install
    install_lottie
}
